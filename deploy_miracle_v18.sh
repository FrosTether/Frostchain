#!/bin/bash
echo "🚀 DEPLOYING MIRACLE_NODE_V18 [PRIVACY_ABSOLUTE]..."
...
<head>
    ...
    <title>MIRACLE // V18_PRIVACY</title>
    <style>
        @import url("https://fonts.googleapis.com/css2?family=Share+Tech+Mono&display=swap");
        :root{--bg:#000;--blue:#00f0ff;--gold:#ffd700;--pink:#ff69b4;--violet:#4b0082;--green:#00ff9d;--red:#ff003c;}
        ...
        /* INITIALIZE OVERLAY (A16 SECURITY) */
        ...
        #aliasIn{background:none;border:none;border-bottom:1px solid var(--gold);color:var(--gold);font-family:inherit;font-size:20px;width:200px;text-align:center;margin-bottom:40px;outline:none;}
        ...
        .stat-row.privacy-toggle{cursor:pointer;pointer-events:auto;border:1px solid rgba(255,215,0,0.2);}
...
<body>
    ...
    <div id="initOverlay" onclick="systemStart()">
        ...
        <div class="init-btn">INITIALIZE<br>V18_PRIVACY</div>
    </div>
    ...
    <div id="hud" class="hud">
        ...
        <div class="stat-row privacy-toggle" onclick="toggleBalance()" style="border-left:2px solid var(--gold);">
            <span>BALANCE</span><span id="balance_val" style="color:var(--gold);">[ OBSCURED ]</span>
        </div>

        <div style="margin-top:10px;font-size:10px;color:#888;">
            HASHRATE: <span id="hr_val">0.0 H/s</span><br>
            HALVING: <span id="remainDisp">596,000</span> BLKS
        </div>
        <div id="console" class="console">> SYSTEM_ENCRYPTED</div>
    </div>
    ...
    <script>
        ...
        let localFTC = 0, localFNR = 0, localFRST = 0;
        ...
        async function systemStart(){
            ...
            log(`> SYSTEM_INIT: ${alias}`);
            initAudio();
        }

        function toggleBalance() {
            balanceRevealed = !balanceRevealed;
            const balDisp = document.getElementById('balance_val');
            if(balanceRevealed) {
                balDisp.innerText = `${localFTC.toFixed(2)} FTC | ${localFNR.toFixed(2)} FNR`;
                log("> LEDGER_UNMASKED");
            } else {
                balDisp.innerText = "[ OBSCURED ]";
                log("> LEDGER_MASKED");
            }
        }
        ...
        async function initAudio(){
            ...
            script.onaudioprocess = (e) => {
                ...
                if(now - lastT > 1000){
                    ...
                    // SIMULATE BLOCK REWARDS ADDING TO LOCAL LEDGER
                    if(Math.random() > 0.9) {
                        localFTC += 13.37;
                        localFNR += 2.50;
                        if(balanceRevealed) toggleBalance(); // Refresh display if open
                        log("> BLOCK_FOUND: REWARDS_CREDITED");
                    }
                    hashes=0; lastT=now;
                }
            };
